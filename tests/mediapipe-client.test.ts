...
function fixture(t: TestContext) {
  ...
  const createClient = (
    ...
    categories: readonly DetectionCategory[] = ["person"],
    scoreThreshold = 0.7,
  ) => {
  ...
  const finish = (frame: FrameRequest) =>
    worker.reply({
      type: "result",
  ...
  const frames = () =>
    worker.sent.filter((message): message is FrameRequest => message.type === "frame");
  t.after(() => {
  ...
  return {
    ...
    delayCapture(value: Promise<void>) {
      captureDelay = value;
    },
  };
}
...
test("正常なFPS間引きとbusyを分け、縮小画像を所有権転送する", async (t) => {
  ...
  assert.deepEqual(f.captures[0].options, {
    resizeWidth: 320,
    resizeHeight: 180,
    resizeQuality: "low",
  });
...
test("推論解像度の変更後は古い結果を破棄し、選択した寸法で転送する", async (t) => {
  ...
  assert.deepEqual(f.captures[1].options, {
    resizeWidth: 320,
    resizeHeight: 180,
    resizeQuality: "low",
  });
...
test("連続する設定変更では古い完了・エラーを無視し、現在の設定エラーは停止後も通知する", async (t) => {
  ...
  f.worker.reply({
    type: "error",
    scope: "configuration",
    configurationId: latest.configurationId,
