...
const redPatch = (x: number, y: number) =>
  x >= 3 && x <= 7 && y >= 3 && y <= 7 ? [255, 0, 0] : [0, 0, 255];

function matches(rgb: readonly number[], settings = DEFAULT_COLOR_SETTINGS): boolean {
  return (
    new ColorDetector(5, 5).process(
      frame(5, 5, () => rgb),
      settings,
    ).mask[12] === 1
  );
...
test("赤は0°と360°をまたいで検出し、許容幅外の色・白・黒を除く", () => {
  ...
  for (const rgb of [
    ...
    assert.equal(matches(rgb, settings), false);
...
test("明るさ・彩度の許容幅は選択色を中心に独立して作用する", () => {
  ...
  assert.equal(
    matches([255, 255, 255], { ...DEFAULT_COLOR_SETTINGS, saturationTolerance: 1 }),
    false,
  );
});
...
for (const kernel of [3, 5]) {
  test(`${kernel}×${kernel} openingで孤立画素を除き、色マスク・バッファを再利用する`, () => {
    ...
    assert.equal(
      result.mask.reduce((sum, value) => sum + value, 0),
      25,
    );
...
test("色条件の無効値・不一致サイズを拒否し、画面全体が対象色でも検出する", () => {
  for (const key of ["hueTolerance", "saturationTolerance", "valueTolerance"] as const) {
    for (const value of [NaN, Infinity, -1, 181]) {
      assert.throws(
        () => matches([255, 0, 0], { ...DEFAULT_COLOR_SETTINGS, [key]: value }),
        RangeError,
      );
    }
  }
  ...
  const makeCanvas = () => {
    const context = {
      ...
      getImageData: (_x: number, _y: number, width: number, height: number) => {
        reads++;
        return frame(width, height, paint);
      },
      clearRect() {},
      setTransform() {},
      save() {},
    ...
    return {
      ...
      height: 0,
      getContext: () => context,
      ownerDocument: { createElement: () => gpu.canvas },
...
test("背景初期化なしで検出し、色変更で追跡をリセット、描画設定変更では継続する", () => {
  ...
  assert.equal(
    f.engine.process(f.video, timestamp, { ...settings, targetColor: "#00ff00" }).trackCount,
    0,
  );
  ...
  assert.equal(
    f.engine.process(f.video, timestamp + 100, { ...settings, minBlobAreaRatio: 0.5 })
      .detectionCount,
    0,
  );
...
test("320/480の切り替え・回転・時間の巻き戻り・中断で状態を初期化する", () => {
  ...
  assert.throws(
    () => f.engine.process(f.video, 0, { ...DEFAULT_COLOR_SETTINGS, minBlobAreaRatio: NaN }),
    RangeError,
  );
});
...
test("全処理区間の計測を記録し、統計だけのリセットではTrackを消さない", (t) => {
  ...
  const summary = f.engine.getTimingSummary();
  for (const key of ["capture", "segmentation", "components", "tracking", "render"] as const)
