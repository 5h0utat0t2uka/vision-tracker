export function webglFixture(
  options: { unavailable?: boolean; shaderFailure?: boolean; linkFailure?: boolean } = {},
) {
  ...
  const record = (name: string, args: unknown[]) =>
    calls.push({
      name,
      args: args.map((arg) =>
        arg instanceof Float32Array || arg instanceof Uint8Array ? arg.slice() : arg,
      ),
    });
  ...
  const canvas = {
    width: 0,
    height: 0,
    getContext: (kind: string) => (kind === "webgl" && !options.unavailable ? gl : null),
