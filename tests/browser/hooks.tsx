...
import { ColorTrackingEngine, INITIAL_COLOR_RESULT } from '../../src/routes/color-segmentation/components/ColorTrackingEngine.ts'
...
const stats = {
  ...
  observers: 0, stops: 0, renders: 0, observations: 0, bitmapsClosed: 0,
  configurations: [] as DetectorWorkerRequest[],
  settings: {} as Record<string, unknown>,
  drawOptions: {} as Record<string, unknown>,
}
for (const [kind, Engine] of [['background', TrackingEngine], ['color', ColorTrackingEngine]] as const) {
  const instances = new WeakSet()
  const resize = Engine.prototype.resizeOverlay
  Engine.prototype.resizeOverlay = function (...args) {
    if (!instances.has(this)) { instances.add(this); stats[kind].instances++ }
    return resize.apply(this, args)
  }
  const reset = Engine.prototype.reset
  Engine.prototype.reset = function () { stats[kind].resets++; reset.call(this) }
  Engine.prototype.process = function (_video: HTMLVideoElement, _time: number, settings: TrackingSettings | ColorTrackingSettings) {
    stats[kind].processes++
    stats.settings = { ...settings }
    return { ...INITIAL_COLOR_RESULT, trackCount: 1, detectionCount: 1, isCalibrating: false, foregroundRatio: 0.1 }
  }
}
...
Heatmap.prototype.observe = function (...args) { stats.observations++; return observe.apply(this, args) }

class TestResizeObserver {
  private observing = false
  observe() { if (!this.observing) stats.observers++; this.observing = true }
  disconnect() { if (this.observing) stats.observers--; this.observing = false }
...
Object.assign(globalThis, {
  createImageBitmap: async () => ({ close() { stats.bitmapsClosed++ } }),
})
...
class TestWorker extends EventTarget {
  ...
  emit(message: DetectorWorkerResponse) { this.dispatchEvent(new MessageEvent('message', { data: message })) }
  postMessage(message: DetectorWorkerRequest) {
    if (message.type === 'init' || message.type === 'configure') {
      stats.configurations.push(message)
      queueMicrotask(() => this.emit({ type: message.type === 'init' ? 'ready' : 'configured', configurationId: message.configurationId }))
  ...
  release() {
    for (const frame of this.pending.splice(0)) this.emit({
      type: 'result', generation: frame.generation, timestampMs: frame.timestampMs,
      width: frame.sourceWidth, height: frame.sourceHeight, inferenceTimeMs: 2,
      detections: [{ categoryName: 'person', score: 0.9, bbox: { x: 100, y: 80, width: 60, height: 100 }, center: { x: 130, y: 130 }, area: 6000 }],
    })
  }
...
const stopCamera = () => { stats.stops++ }
const common = {
  videoRef, stageRef, heatmap, stopCamera,
  analysisRef, filterRef, overlayRef,
  analysisCanvasRef: analysisRef, filterCanvasRef: filterRef, overlayCanvasRef: overlayRef,
}
let options = {
  ...common, cameraStatus: 'idle' as CameraStatus,
  settings: { ...DEFAULT_COLOR_SETTINGS, motionThreshold: 70, backgroundTimeConstantMs: 3300 },
  targetFps: 30, analysisLongEdge: 320 as const, longEdge: 320 as const,
  categories: ['person'] as const, scoreThreshold: 0.5,
  inferenceConfiguration: DEFAULT_INFERENCE_CONFIGURATION, inferenceLongEdge: 320 as const,
  inferenceFps: 10, showTrail: true, regionEffect: 'none' as const,
}
let latest: unknown
function Stage() {
  return <section ref={stageRef}><video ref={videoRef} /><canvas ref={analysisRef} /><canvas ref={filterRef} /><canvas ref={overlayRef} /></section>
}
function Background() { latest = useBackgroundTracking(options); return <Stage /> }
function Color() { latest = useColorTracking(options); return <Stage /> }
function MediaPipe() { latest = useMediaPipeTracking(options); return <Stage /> }
...
const harness = {
  async mount(mode: 'background' | 'color' | 'mediapipe') {
    ...
    await act(async () => root.render(<StrictMode><Component /></StrictMode>))
  },
  ...
  async frame(delta = 100) {
    ...
    await act(async () => {
      ...
      frames.clear()
      for (const callback of callbacks) callback(time, { presentationTime: time, presentedFrames: frameId } as VideoFrameCallbackMetadata)
    })
  },
  async release() { await act(async () => { for (const worker of workers) worker.release() }) },
  async settle() { await act(async () => { await new Promise(resolve => setTimeout(resolve, 180)) }) },
  ...
  async resize() { await act(async () => videoRef.current?.dispatchEvent(new Event('resize'))) },
  async unmount() { await act(async () => root.unmount()) },
  async lateCallbacks() {
    await act(async () => { for (const callback of cancelled) callback(time + 100, {} as VideoFrameCallbackMetadata) })
  },
  snapshot() {
    return { ...stats, latest, pendingFrames: frames.size, workers: workers.length, activeWorkers: workers.filter(w => !w.terminated).length, pendingResults: workers.reduce((n, w) => n + w.pending.length, 0) }
  },
}
