...
declare global {
  interface Window { harness: HookHarness }
}

async function snapshot(page: Page) { return page.evaluate(() => window.harness.snapshot()) }
async function update(page: Page, options: Record<string, unknown>) {
  await page.evaluate(next => window.harness.update(next), options)
}
async function frame(page: Page) { await page.evaluate(() => window.harness.frame(600)) }
...
for (const mode of ['background', 'color'] as const) {
  test(`${mode}: StrictMode, settings, frame delivery and cleanup`, async ({ page }) => {
    ...
    await update(page, { settings: { ...started.settings, showTrail: false, regionEffect: 'grayscale' }, targetFps: 15 })
    expect((await snapshot(page))[mode].resets).toBe(started[mode].resets)
    await frame(page)
    expect((await snapshot(page)).settings).toMatchObject({ showTrail: false, regionEffect: 'grayscale' })
...
test('MediaPipe: model lifetime, debouncing, latest display settings and measurements', async ({ page }) => {
  ...
  await page.evaluate(() => window.harness.release())
  await frame(page)
  expect((await snapshot(page)).latest).toMatchObject({ metrics: { trackCount: 1, detectionCount: 1 } })
  ...
  expect((await snapshot(page)).drawOptions).toEqual({ showTrail: false, regionEffect: 'grayscale' })
