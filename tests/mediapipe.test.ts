...
import { convertMediaPipeDetections } from "../src/routes/mediapipe-tasks-vision/lib/convertDetections.ts";
import {
...
import { TIMING_LABELS } from "../src/routes/mediapipe-tasks-vision/lib/timingConfig.ts";
...
test("初期カテゴリは人物で、選択肢とMediaPipe asset URLは固定設定に従う", () => {
  ...
  assert.deepEqual(
    DETECTION_CATEGORIES.map((category) => category.value),
    ["person", "car", "bicycle"],
  );
...
test("Trackerは近接していても異なる検出カテゴリを同じIDへ関連付けない", () => {
  ...
  assert.equal(
    tracker
      .update([detection("car", 108)], 300, TRACKER_SETTINGS)
      .find((track) => track.categoryName === "car")?.id,
    2,
  );
});
...
test("配置したEfficientDet-Lite0モデルは記録済みSHA-256と一致する", async () => {
  const model = await readFile(
    new URL("../public/mediapipe/models/efficientdet-lite0-int8-v1.tflite", import.meta.url),
  );
  assert.equal(
...
test("GPU用float16 v1モデルのSHA-256とTFLite識別子を検証する", async () => {
  const model = await readFile(
    new URL("../public/mediapipe/models/efficientdet-lite0-float16-v1.tflite", import.meta.url),
  );
...
test("初期値と4種類のモデル構成を検証する", () => {
  ...
  assert.deepEqual(
    resolveMediaPipeAssetUrls("/demo/", "https://example.com", "lite0-gpu-float16"),
    {
...
test("推論画像は拡大せず既定モデルの推奨長辺320に収め、丸め誤差を含め元映像座標へ戻す", () => {
  for (const [width, height] of [
    ...
    assert.ok(
      size.width <= DEFAULT_INFERENCE_LONG_EDGE && size.height <= DEFAULT_INFERENCE_LONG_EDGE,
    );
...
test("推論解像度の全選択肢は縦横比を維持し、小さい入力は拡大しない", () => {
  ...
  for (const longEdge of INFERENCE_LONG_EDGES) {
    ...
    assert.deepEqual(getInferenceSize(1280, 720, longEdge), {
      width: longEdge,
      height: (longEdge * 9) / 16,
    });
    assert.deepEqual(getInferenceSize(720, 1280, longEdge), {
      width: (longEdge * 9) / 16,
      height: longEdge,
    });
...
test("EfficientDet-Lite2モデルの配布ファイルとURLを検証する", async () => {
  ...
  for (const model of models) {
    const contents = await readFile(
      new URL("../public/mediapipe/models/" + model.fileName, import.meta.url),
    );
    assert.equal(contents.subarray(4, 8).toString(), "TFL3");
    assert.equal(createHash("sha256").update(contents).digest("hex"), model.sha256);
    assert.deepEqual(
      resolveMediaPipeAssetUrls("/demo/", "https://example.com", model.configuration),
      {
