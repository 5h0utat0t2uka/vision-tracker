...
test("False colorのRGBAテクスチャは配色定数と不均等な区間の補間を維持する", () => {
  ...
  for (const [index, stop] of FALSE_COLOR_STOPS.entries()) {
    ...
    assert.deepEqual(
      [...FALSE_COLOR_PALETTE.slice(stop.brightness * 4, stop.brightness * 4 + 4)],
      [...stop.color, 255],
    );
  }
  for (const [brightness, rgb] of [
    [40, [0, 100, 218]],
    [115, [0, 200, 128]],
    [175, [128, 228, 0]],
    [222, [255, 153, 0]],
    ...
    assert.deepEqual(
      [...FALSE_COLOR_PALETTE.slice(brightness * 4, brightness * 4 + 4)],
      [...rgb, 255],
    );
  }
});
...
function track(x = 0, state: Track["state"] = "confirmed"): Track {
  const bbox = { x, y: 0, width: 160, height: 90 },
    center = { x: x + 80, y: 45 };
  return {
    id: 1,
    bbox,
...
test("WebGLは全矩形を1回の動画転送で描き、移動・消失時に毎回クリアする", () => {
  const f = webglFixture(),
    renderer = new FalseColorRenderer(f.canvas);
...
test("WebGLの初期化失敗・コンテキスト消失は明示的なエラーとなる", () => {
  assert.throws(
    () => new FalseColorRenderer(webglFixture({ unavailable: true }).canvas),
    /requires WebGL/,
  );
