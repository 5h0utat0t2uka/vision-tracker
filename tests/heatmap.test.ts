...
function track(time: number, x = 32, id = 1, state: Track['state'] = 'confirmed', scale = 1): Track {
  const box = { x: x * scale, y: 32 * scale, width: 16 * scale, height: 16 * scale }
  const center = { x: (x + 8) * scale, y: 40 * scale }
  return {
    id, state, hits: 2, bbox: box, center, lastObservedBox: box, lastObservedCenter: center,
    lastObservedAtMs: time, velocity: { x: 0, y: 0 }, trail: [],
  }
}
...
test('stationary rectangle occupancy uses seconds, independently of 5/10/15 FPS', () => {
  for (const fps of [5, 10, 15]) {
    ...
    for (let i = 0; i <= fps * 2; i++) heatmap.observe([track(i * 1000 / fps)], i * 1000 / fps, 128, 128)
  ...
  for (const fps of [5, 10, 15]) {
    for (const scale of [1, 2, 4]) {
      ...
      for (let i = 0; i <= fps; i++) {
        ...
        heatmap.observe([track(time, 16 + 64 * i / fps, 1, 'confirmed', scale)], time, 128 * scale, 128 * scale)
      }
...
test('overlapping rectangles add target-seconds; edge coverage is clipped', () => {
  const heatmap = new Heatmap()
  for (const time of [0, 1000]) heatmap.observe([track(time, -8, 1), track(time, -8, 2)], time, 128, 128)
...
test('drawing reuses the heat texture and cover coordinates without changing measurements', () => {
  ...
  const offscreen = {
    ...
    putImageData: (data: ImageData) => { image = data; uploads++ },
  }
  const canvas = { width: 0, height: 0, getContext: () => offscreen }
  const document = { createElement: () => canvas } as unknown as Document
  const draws: unknown[][] = []
  const context = { save() {}, restore() {}, drawImage: (...args: unknown[]) => draws.push(args) } as unknown as CanvasRenderingContext2D
  const before = heatmap.values('occupancy')
