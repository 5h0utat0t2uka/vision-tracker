...
test("MotionDetectorは背景初期化後に3×3の動体領域を検出する", () => {
  ...
  const foreground = imageData(width, height, [6, 7, 8, 11, 12, 13, 16, 17, 18]);
...
for (const kernelSize of [3, 5]) {
  test(`${kernelSize}×${kernelSize} openingは小領域・細い動体を除去し、カーネル以上の矩形を残す`, () => {
    const width = 24;
    const height = 20;
    for (const [regionWidth, regionHeight] of [
      [1, 1],
      [3, 3],
      [5, 5],
      [2, 10],
  ...
  test(`${kernelSize}×${kernelSize}の分離処理は画面端・小さい画像を含め2次元の参照実装と一致する`, () => {
    ...
    for (const [width, height] of [
      ...
      for (let sample = 0; sample < 20; sample += 1) {
        ...
        assert.deepEqual(
          result.mask,
          referenceOpening(input, width, height, kernelSize),
          `${width}×${height}, sample ${sample}`,
        );
      }
    }
  });
  ...
  for (let index = 0; index <= 90; index += 1) {
    ...
    if (scheduler.shouldProcess(timestampMs, fps)) {
      const tracks = tracker.update(
        [detectionAt(20 + timestampMs * 0.04, 20)],
        timestampMs,
        SETTINGS,
      );
...
function imageData(width: number, height: number, whitePixelIndexes: readonly number[]): ImageData {
