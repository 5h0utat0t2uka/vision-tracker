...
test('One Euro Filter follows the adaptive cutoff equations with irregular timestamps', () => {
  ...
  let derivative = 0
  let previousTime = 0
  for (const [timestamp, input] of [[33, 0.21], [100, 0.19], [300, 0.35], [350, 0.4]]) {
...
test('invalid options and samples are rejected without poisoning state', () => {
  for (const options of [
    { ...OPTIONS, minCutoffHz: 0 }, { ...OPTIONS, beta: -1 },
    { ...OPTIONS, derivativeCutoffHz: Number.NaN }, { ...OPTIONS, beta: Infinity },
