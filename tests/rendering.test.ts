...
test("Region effectは定義済みの選択肢だけを受け付ける", () => {
  assert.deepEqual(
    REGION_EFFECT_OPTIONS.map((option) => option.value),
    ["grayscale", "invert", "greenscale", "false-color-webgl", "none"],
  );
...
test("背景差分は実行した区間だけ計測し、重複フレームは統計を増やさずリセットで消去する", (t) => {
  ...
  for (let timestamp = 100; timestamp <= 800; timestamp += 100)
    engine.process(video, timestamp, SETTINGS);
...
for (const [sourceWidth, sourceHeight, longEdge] of [
  ...
  test(`${sourceWidth}x${sourceHeight}・長辺${longEdge}のcover座標は切り抜き映像と枠で一致する`, () => {
    ...
    const destination = [
      (640 - sourceWidth * scale) / 2,
      (480 - sourceHeight * scale) / 2,
      sourceWidth * scale,
      sourceHeight * scale,
    ];
    ...
    renderer.render([{ ...track, state: "lost" }], video, {
      showTrail: false,
      regionEffect: "grayscale",
    });
