...
export class FalseColorRenderer {
  ...
  constructor(canvas: HTMLCanvasElement) {
    ...
    if (!gl)
      throw new Error(
        "False color requires WebGL. Select another Region effect and restart the camera.",
      );
    this.gl = gl;
    try {
      this.initialize();
  ...
  private initialize(): void {
    ...
    try {
      for (const [type, source] of [
        ...
        if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS))
          throw new Error(`False color shader compilation failed: ${gl.getShaderInfoLog(shader)}`);
      }
      ...
      if (!gl.getProgramParameter(this.program, gl.LINK_STATUS))
        throw new Error(
          `False color program linking failed: ${gl.getProgramInfoLog(this.program)}`,
        );
    ...
    if (!this.buffer || !this.videoTexture || !this.paletteTexture)
      throw new Error("Failed to allocate False color GPU resources.");
    ...
    for (const [unit, texture, name] of [
      [0, this.videoTexture, "videoTexture"],
      [1, this.paletteTexture, "paletteTexture"],
    ] as const) {
  ...
  render(
    ...
    if (this.disposed || gl.isContextLost())
      throw new Error(
        "False color WebGL context was lost. Restart the camera or select another Region effect.",
      );
