...
for (let brightness = 0; brightness < 256; brightness++) {
  while (
    segment < FALSE_COLOR_STOPS.length - 2 &&
    brightness > FALSE_COLOR_STOPS[segment + 1].brightness
  ) {
    segment += 1;
  }
  ...
  for (let channel = 0; channel < 3; channel++) {
    FALSE_COLOR_PALETTE[brightness * 4 + channel] = Math.round(
      start.color[channel] + (end.color[channel] - start.color[channel]) * weight,
    );
  }
