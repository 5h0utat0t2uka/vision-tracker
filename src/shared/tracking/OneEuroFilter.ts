...
export class OneEuroFilter {
  ...
  constructor(options: OneEuroFilterOptions) {
    if (
      !Number.isFinite(options.minCutoffHz) ||
      options.minCutoffHz <= 0 ||
      !Number.isFinite(options.beta) ||
      options.beta < 0 ||
      !Number.isFinite(options.derivativeCutoffHz) ||
      options.derivativeCutoffHz <= 0
    ) {
  ...
  filter(value: number, timestampMs: number): number {
    ...
    this.derivative +=
      smoothingWeight(this.options.derivativeCutoffHz, elapsedSeconds) *
