...
export class Heatmap {
  ...
  observe(tracks: readonly Track[], time: number, sourceWidth: number, sourceHeight: number): void {
    if (![time, sourceWidth, sourceHeight].every(Number.isFinite) || sourceWidth <= 0 || sourceHeight <= 0) return
    ...
    for (const track of tracks) {
      ...
      if (![box.x, box.y, box.width, box.height, center.x, center.y].every(Number.isFinite) || box.width <= 0 || box.height <= 0) continue
      const observation: Observation = {
        time, x: center.x / sourceWidth, y: center.y / sourceHeight,
        box: { x: box.x / sourceWidth, y: box.y / sourceHeight, width: box.width / sourceWidth, height: box.height / sourceHeight },
      }
      ...
      for (let step = 0; step < steps; step++) {
        ...
        if (x >= 0 && x < width && y >= 0 && y < height) this.movement[y * width + x] += distance / steps
      }
    }
  ...
  private addRectangle(box: Rect, seconds: number): void {
    ...
    for (let y = Math.floor(top); y < Math.ceil(bottom); y++) {
      for (let x = Math.floor(left); x < Math.ceil(right); x++) {
        const coverage = (Math.min(x + 1, right) - Math.max(x, left)) * (Math.min(y + 1, bottom) - Math.max(y, top))
        this.occupancy[y * this.width + x] += seconds * coverage
      }
    }
  }

  draw(context: CanvasRenderingContext2D, document: Document, x: number, y: number, width: number, height: number): void {
      ...
      for (let row = 0; row < this.height; row++) {
        for (let column = 0; column < this.width; column++) {
          ...
          for (let channel = 0; channel < 3; channel++) {
            data.data[offset + channel] = COLORS[segment][channel] * (1 - fraction) + COLORS[segment + 1][channel] * fraction
          }
