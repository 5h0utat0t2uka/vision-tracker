...
import { ObjectDetectorClient, type ObjectDetectorResult } from '../components/ObjectDetectorClient.ts'
import {
  METRICS_REPORT_INTERVAL_MS, TRACK_MISSING_TOLERANCE_MS, resolveMediaPipeAssetUrls,
  type DetectionCategory, type InferenceConfiguration, type InferenceLongEdge,
...
export function useMediaPipeTracking({
  videoRef, filterCanvasRef, overlayCanvasRef, stageRef, heatmap, cameraStatus, stopCamera,
  categories, scoreThreshold, inferenceConfiguration, inferenceFps, inferenceLongEdge,
  showTrail, regionEffect,
  ...
  useEffect(() => {
    ...
    const processFrame: VideoFrameRequestCallback = (now, metadata) => {
      ...
      if (document.visibilityState === 'visible') {
        ...
        void detector.submitFrame(video, metadata.presentationTime, current.inferenceFps, inferenceLongEdge)
  ...
  }, [cameraStatus, stopCamera, detectorStatus, inferenceConfiguration, timings, inferenceLongEdge, heatmap, videoRef, filterCanvasRef, overlayCanvasRef, stageRef])
