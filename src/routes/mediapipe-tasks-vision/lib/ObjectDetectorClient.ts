...
import type { DetectorWorkerRequest, DetectorWorkerResponse } from "./protocol.ts";
...
export class ObjectDetectorClient {
  ...
  constructor(callbacks: ObjectDetectorClientCallbacks) {
    ...
    this.worker = new Worker(new URL("./object-detector.worker.ts", import.meta.url), {
      type: "module",
    });
      ...
      this.worker.postMessage(
        {
          ...
          sourceWidth,
          sourceHeight,
        } satisfies DetectorWorkerRequest,
        [bitmap],
      );
      bitmap = null;
    } catch (error) {
