...
export const INITIAL_COLOR_RESULT: ColorFrameResult = {
  trackCount: 0,
  detectionCount: 0,
  matchedRatio: 0,
};
...
export class ColorTrackingEngine {
  ...
  syncVideoSize(video: HTMLVideoElement, longEdge = this.longEdge): void {
    ...
    if (
      ...
      this.pipeline.kernelSize === kernelSize
    )
      return;
  ...
  process(
    ...
    if (
      !Number.isFinite(settings.minBlobAreaRatio) ||
      settings.minBlobAreaRatio < 0 ||
      settings.minBlobAreaRatio > 1
    ) {
    ...
    if (
      ...
      (timestampMs < this.previousTimestamp ||
        timestampMs - this.previousTimestamp > MAX_FRAME_GAP_MS)
    )
      this.reset();
    ...
    const detections = pipeline.components.extract(
      color.mask,
      Math.max(1, Math.round(pipeline.width * pipeline.height * settings.minBlobAreaRatio)),
    );
