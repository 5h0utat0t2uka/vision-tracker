...
import {
  ACHROMATIC_SATURATION_LIMIT,
  DARK_VALUE_LIMIT,
  type ColorDetectionSettings,
} from "./config.ts";
...
export class ColorDetector {
  ...
  process(
  ...
  ): { mask: Uint8Array; matchedRatio: number } {
    if (
      frame.width !== this.width ||
      frame.height !== this.height ||
      frame.data.length !== this.mask.length * 4
    ) {
      throw new RangeError("Unexpected color analysis frame dimensions.");
    }
    for (const [value, limit] of [
      [settings.hueTolerance, 180],
      [settings.saturationTolerance, 1],
      [settings.valueTolerance, 1],
    ]) {
      if (!Number.isFinite(value) || value < 0 || value > limit)
        throw new RangeError("Invalid color tolerance.");
    }
    ...
    for (let index = 0, offset = 0; index < mask.length; index++, offset += 4) {
      ...
      let matches =
        frame.data[offset + 3] === 255 && Math.abs(pixel.v - target.v) <= settings.valueTolerance;
      if (matches && mode !== "dark") {
        ...
        if (matches && mode === "chromatic") {
          ...
          matches =
            pixel.s > ACHROMATIC_SATURATION_LIMIT &&
            pixel.v > DARK_VALUE_LIMIT &&
