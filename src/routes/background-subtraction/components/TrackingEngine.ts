...
export class TrackingEngine {
  ...
  constructor(
    ...
    this.overlayRenderer = new OverlayRenderer(filterCanvas, overlayCanvas, 1, 1, heatmap);
  }
  ...
  process(video: HTMLVideoElement, timestampMs: number, settings: TrackingSettings): FrameResult {
    ...
    const elapsedMs =
      this.previousTimestampMs === null ? 0 : timestampMs - this.previousTimestampMs;
    this.previousTimestampMs = timestampMs;
    const startedAt = performance.now();
    this.analysisContext.drawImage(video, 0, 0, pipeline.width, pipeline.height);
    const frame = this.analysisContext.getImageData(0, 0, pipeline.width, pipeline.height);
    ...
    const minimumArea = Math.max(
      1,
      Math.round(pipeline.width * pipeline.height * settings.minBlobAreaRatio),
    );
    const componentsStartedAt = performance.now();
    const detections = pipeline.connectedComponents.extract(motion.mask, minimumArea);
