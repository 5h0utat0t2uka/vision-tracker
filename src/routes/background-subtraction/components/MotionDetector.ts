import { timeConstantFrom30FpsRate, timeWeight } from "../../../shared/tracking/timing.ts";
...
export class MotionDetector {
  ...
  process(
    ...
    for (let index = 0; index < this.gray.length; index += 1) {
      ...
      const isForeground =
        difference >= threshold || (wasForeground && difference >= releaseThreshold);
    ...
    for (let index = 0; index < this.gray.length; index += 1) {
      const learningRate =
        this.rawMask[index] === 1 ? foregroundLearningRate : backgroundLearningRate;

      this.background[index] += learningRate * (this.gray[index] - this.background[index]);
    }
  ...
  private toGrayscale(source: Uint8ClampedArray): void {
    for (
      ...
      this.gray[targetIndex] =
        (77 * source[sourceIndex] + 150 * source[sourceIndex + 1] + 29 * source[sourceIndex + 2]) >>
        8;
    }
  }
  ...
  private result(isCalibrating: boolean, foregroundRatio: number): MotionDetectionResult {
    return {
      mask: this.rawMask,
      isCalibrating,
      foregroundRatio,
